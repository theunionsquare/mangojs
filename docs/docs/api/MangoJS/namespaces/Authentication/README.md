[**MangoJS**](../../../README.md)

***

# Authentication

## Description

Handles all authentication logic including login, JWT, and OAuth strategies.

## Other

- [AuthContext](classes/AuthContext.md)
- [AuthenticationError](classes/AuthenticationError.md)
- [AuthStrategyRegistry](classes/AuthStrategyRegistry.md)
- [ApiKeyStrategyOptions](interfaces/ApiKeyStrategyOptions.md)
- [AuthCookieOptions](interfaces/AuthCookieOptions.md)
- [AuthCredentials](interfaces/AuthCredentials.md)
- [GenerateTokenPayload](interfaces/GenerateTokenPayload.md)
- [IAuthContext](interfaces/IAuthContext.md)
- [IAuthUser](interfaces/IAuthUser.md)
- [JWTStrategyOptions](interfaces/JWTStrategyOptions.md)

## Strategies

- [ApiKeyStrategy](classes/ApiKeyStrategy.md)
- [BaseAuthStrategy](classes/BaseAuthStrategy.md)
- [JWTStrategy](classes/JWTStrategy.md)
- [IAuthStrategy](interfaces/IAuthStrategy.md)
- [AUTH\_STRATEGY\_TAG](variables/AUTH_STRATEGY_TAG.md)
