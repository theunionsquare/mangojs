# External Resources

This page consolidates all external documentation references used throughout MangoJS.

---

## Core Dependencies

### TypeORM
- **Official Documentation**: [https://typeorm.io/](https://typeorm.io/)
- **Entities Guide**: [https://typeorm.io/entities](https://typeorm.io/entities)
- **Column Types Reference**: [https://typeorm.io/entities#column-types](https://typeorm.io/entities#column-types)
- **EntityManager API**: [https://typeorm.io/entity-manager-api](https://typeorm.io/entity-manager-api)

### Inversify (Dependency Injection)
- **Official Documentation**: [https://inversify.io/](https://inversify.io/)

### Express
- **Official Documentation**: [https://expressjs.com/](https://expressjs.com/)

---

## API Documentation

### Swagger/OpenAPI
- **OpenAPI 3.0 Specification**: [https://swagger.io/specification/](https://swagger.io/specification/)
- **Swagger Editor**: [https://editor.swagger.io/](https://editor.swagger.io/)

---

## TypeScript

### TypeScript Official
- **Official Documentation**: [https://www.typescriptlang.org/docs/](https://www.typescriptlang.org/docs/)
- **Utility Types**: [https://www.typescriptlang.org/docs/handbook/utility-types.html](https://www.typescriptlang.org/docs/handbook/utility-types.html)
- **Decorators**: [https://www.typescriptlang.org/docs/handbook/decorators.html](https://www.typescriptlang.org/docs/handbook/decorators.html)

---

## MangoJS Examples

### Reference Implementations
- **IAM Service**: `src/services/iam_server/src/`
  - Complete microservice example with authentication
  - Database models: `src/services/iam_server/src/db/models/`
  - Services: `src/services/iam_server/src/services/`
  - Controllers: `src/services/iam_server/src/routes/`
  - Types: `src/services/iam_server/src/types/`

- **Sample Service**: `src/services/sample/src/`
  - Minimal service template
  - Quick start reference
