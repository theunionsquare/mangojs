...
export function OrAuth(): MethodDecorator {
  return function (
    ...
    if (validators && validators.length > 0) {
      console.log(
        `[OrAuth] Found ${validators.length} validators, creating OR orchestrator`,
      );

      // Import the orchestrator
      const {
        createAuthOrchestrator,
      } = require("../../authz/authOrchestrator");
      ...
      console.warn(
        `[OrAuth] No validators found for method: ${String(propertyKey)}`,
      );
    }
