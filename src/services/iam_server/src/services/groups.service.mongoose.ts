...
export class GroupsService {
    ...
    public async getAllGroups(filter: {}): Promise<{}> {
        const response = await this._persistenceContext.inTransaction(
            async (em: mongoose.Connection) => {
                const response: Array<APITYPE.V1.groups.ResponseBodyData> = []
    ...
    public async getGroups(userType: Types.entities.AuthUserType): Promise<{}> {
        const response = await this._persistenceContext.inTransaction(
            async (em: mongoose.Connection) => {
                const response: Array<APITYPE.V1.groups.admin.ResponseBodyData> =
                    []
    ...
    public async postGroup(
        ...
        adminGroup: APITYPE.V1.groups.ResponseBodyData
