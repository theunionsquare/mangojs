...
import PartnerUserSchema, { IPartnerUser } from '../db/models/PartnerUser.model'
...
export class PartnerUserService {
    ...
    public async getPartnerUsers(): Promise<{}> {
        const response = await this._persistenceContext.inTransaction(
            async (em: mongoose.Connection) => {
                ...
                const filter = {}
                const users =
                    await PartnerUserSchema.find(filter).populate('groups')
                //console.log({ users });
    ...
    public async postPartnerUser(
        partnerUser: APITYPE.V1.adminUser.POST.RequestBody
        ...
        const response = (await this._persistenceContext.inTransaction(
            async (em: mongoose.Connection) => {
                ...
                const magicLinkExpireDate = expDate.setDate(
                    expDate.getDate() + 1
                )
                // create admin user
