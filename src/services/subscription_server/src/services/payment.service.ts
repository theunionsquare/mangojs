...
export class PaymentService {
     ...
     */
    public async cancelSubscription(externalSubscriptionId: string): Promise<void> {
        try {
            // Try Stripe first
        ...
        return new Promise((resolve, reject) => {
            ...
            paypal.billingPlan.create(subscriptionData, (error, subscription) => {
                if (error) {
                    ...
                    resolve({
                        id: subscription.id,
                        customerId: subscription.subscriber?.subscriber_id || subscription.id
                    })
                }
            })
        })
    ...
    private async cancelPayPalSubscription(subscriptionId: string): Promise<void> {
        return new Promise((resolve, reject) => {
            paypal.billingAgreement.cancel(subscriptionId, { note: 'Subscription canceled by user' }, (error, response) => {
                if (error) {
                    reject(error)
                } else {
                    resolve()
