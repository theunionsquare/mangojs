...
export class SubscriptionService {
  ...
  public async getUserSubscriptions(userId: string): Promise<Subscription[]> {
    const response = await this._persistenceContext.inTransaction(
      async (em: EntityManager) => {
        const subscriptions = await em.getRepository(Subscription).find({
          where: { userId },
          relations: ["plan"],
        });
  ...
  public async getSubscriptionById(
    ...
    const response = await this._persistenceContext.inTransaction(
      async (em: EntityManager) => {
        const subscription = await em.getRepository(Subscription).findOne({
          where: { uid: subscriptionId },
          relations: ["plan"],
        });

        if (!subscription) {
          throw new errors.APIError(404, "NOT_FOUND", "Subscription not found");
        }
        return subscription;
      }
    );
  ...
  public async createSubscription(
    data: api.v1.subscriptions.POST.RequestBody
  ): Promise<Subscription> {
    const response = await this._persistenceContext.inTransaction(
      async (em: EntityManager) => {
        ...
        const savedSubscription = await em
  ...
  public async updateSubscription(
    subscriptionId: string,
    data: api.v1.subscriptions.PUT.RequestBody
  ): Promise<Subscription> {
    const response = await this._persistenceContext.inTransaction(
      async (em: EntityManager) => {
        const subscription = await em.getRepository(Subscription).findOne({
  ...
  public async cancelSubscription(
    ...
    const response = await this._persistenceContext.inTransaction(
      async (em: EntityManager) => {
        ...
        const canceledSubscription = await em
          .getRepository(Subscription)
          .save(subscription);
  ...
  public async pauseSubscription(
    ...
    const response = await this._persistenceContext.inTransaction(
      async (em: EntityManager) => {
        ...
        const pausedSubscription = await em
          .getRepository(Subscription)
          .save(subscription);
  ...
  public async resumeSubscription(
    ...
    const response = await this._persistenceContext.inTransaction(
      async (em: EntityManager) => {
        ...
        const resumedSubscription = await em
          .getRepository(Subscription)
          .save(subscription);
