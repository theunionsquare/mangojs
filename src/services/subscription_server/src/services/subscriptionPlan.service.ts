...
export class SubscriptionPlanService {
  ...
  public async getActivePlans(): Promise<SubscriptionPlan[]> {
    const response = await this._persistenceContext.inTransaction(
      async (em: Connection) => {
        const plans = await em.getRepository(SubscriptionPlan).find({
          where: { isActive: true },
          order: { price: "ASC" },
        });
        return plans;
  ...
  public async createPlan(
    data: api.v1.plans.POST.RequestBody
  ...
  public async updatePlan(
    planId: string,
    data: api.v1.plans.PUT.RequestBody
  ): Promise<SubscriptionPlan> {
    const response = await this._persistenceContext.inTransaction(
      async (em: Connection) => {
        const plan = await em
        ...
        if (data.billingInterval !== undefined)
          plan.billingInterval = data.billingInterval;
  ...
  public async deletePlan(planId: string): Promise<void> {
    await this._persistenceContext.inTransaction(async (em: Connection) => {
      const plan = await em
        .getRepository(SubscriptionPlan)
        .findOne({ where: { uid: planId } });
